...
class Groups extends Admin_Controller
{
  ...
  {
    $data['heading_title'] = 'Nhóm quyền';
    $data['heading_description'] = "Quản lý nhóm quyền";
    /*Breadcrumbs*/
  ...
  public function ajax_list()
  {
    if ($this->input->server('REQUEST_METHOD') == 'POST' && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
      ...
      if (!empty($list)) foreach ($list as $item) {
        ...
        $action = in_array($item->id, [1, 2]) ? ['edit'] : ['edit', 'delete'];
    ...
    if ($this->input->server('REQUEST_METHOD') == 'POST' && !empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
      $rules = [
        [
          'field' => 'name',
          'label' => 'Tên nhóm',
          'rules' => 'required|min_length[3]|max_length[50]|trim|xss_clean|callback_validate_html'
        ],
        [
          'field' => 'description',
          'label' => 'Mô tả',
          // 'rules' => 'min_length[3]|max_length[50]|trim|xss_clean|callback_validate_html'
        ],
      ];
