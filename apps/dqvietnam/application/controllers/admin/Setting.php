...
class Setting extends Admin_Controller
{
    ...
    {
        $path = FCPATH . 'config' . DIRECTORY_SEPARATOR;
        $conf = 'settings.cfg';
        $dataContent = file_get_contents($path . $conf);
        ...
        if (!empty($dataPost)) {
            ...
            if (!empty($data_store)) {
                if (file_put_contents($path . $conf, json_encode($data_store))) {
        ...
        //dd($data);
        $data['main_content'] = $this->load->view($this->template_path . 'config/index', $data, TRUE);
        $this->load->view($this->template_main, $data);
    }


    public function ajax_load_item($type = '')
    {
        $this->checkRequestPostAjax();
        $data = $this->input->post();
        print $this->load->view($this->template_path . 'setting/items/_item_' . $type, $data, TRUE);
        exit;
    }
    ...
    {
        $data = array_unique(getProvider(null, true));
        $list = [];
        if (!empty($data)) {
