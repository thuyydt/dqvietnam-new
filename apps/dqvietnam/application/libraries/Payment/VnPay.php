...
class VnPay
{
    ...
    public function __construct(){
        ...
        $this->_vnp_TmnCode = $this->ci->config->item('vnpay')['payment_vnPay_tmnCode'];
        $this->_vnp_HashSecret = $this->ci->config->item('vnpay')['payment_vnPay_hashSecret'];
        $this->_url_API_send = $this->ci->config->item('vnpay')['payment_vnPay_apiSend'];
        $this->_url_API_response = $this->ci->config->item('vnpay')['payment_vnPay_apiResponse'];
        $this->_url_return = $this->ci->config->item('vnpay')['payment_vnPay_return'];
        $this->_type = $this->ci->config->item('vnpay')['payment_vnPay_type'];
    }
                ...
                if (!empty($trans)) {
                    if($trans->status_payment == 1 || $trans->status_payment == 2) {
                        ...
                        if ($request['vnp_ResponseCode'] == '00') {
                            ...
                            $info_pay['trans_bank'] = $request['vnp_BankTranNo'];
                            $info_pay['transaction'] = $request['vnp_TransactionNo'];
                            $info_pay['bank_code'] = $request['vnp_BankCode'];
                            $info_pay['card_type'] = $request['vnp_CardType'];
                            $info_pay['response'] = json_encode($request);
