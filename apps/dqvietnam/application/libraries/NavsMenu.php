...
class NavsMenu
{
  ...
  private $uri_segment = 1;
     ...
     * @var bool
     */
    private $item_active_child = false;
  ...
  private function render_item($items, &$html = '')
  {
    ...
    foreach ($items as $item) {
      ...
      if (isset($item[$this->menu_label], $item[$this->menu_key])) {
        ...
        $html .= $this->set_active($tag_open, $item['slug'],$item['order']);
        ...
        if ($has_children) {
          ...
          if($position ==1 && $item['level'] ==0 && !empty($this->_additional_item['mega'])){
                $html .= $this->_additional_item['mega'];
          }
   ...
   */
  private function set_active($html, $slug,$location=null)
  {
    $segment = $this->ci->uri->segment($this->uri_segment);
    if (($this->item_active != '' && $slug == $this->item_active && empty($segment)) || $slug == $segment
        || ($this->item_active != '' && $slug == $this->item_active && $this->item_active_child)
        || ($this->location_active != null  && $location== $this->location_active)) {
      $doc = new DOMDocument();
      $doc->loadHTML($html);
