<template>
  ...
  <div class="slider" v-if="isStart">
    ...
    <div class="page" v-if="!loading">
      <Slide v-bind:audioBg="audioBackground" v-bind:checkAns="checkAnswer" :key="key" v-if="typeGame==0"></Slide>

      <Question v-bind:audioBg="audioBackground" v-bind:checkAns="checkAnswer" :key="key"
                v-if="typeGame==1"></Question>

      <Fill v-bind:checkAns="checkAnswer" :key="key" v-if="typeGame==2"></Fill>
      ...
      <Card v-bind:audioBg="audioBackground" v-bind:fnChangeGame="changeGame" :key="key"
            v-if="typeGame==5"></Card>

      <Chat v-bind:checkAns="checkAnswer" v-bind:fnChangeGame="changeGame" :key="key" v-if="typeGame==6"></Chat>
...
export default {
  ...
  components: {Slide, Question, Fill, Crossword, Chat, Card, Images},
  methods: {
  ...
  mounted() {
    ...
    //     _this.audioBackground.play();
    // };
    //
    // window.onblur = function() {
    //     if (_this.isStart) _this.audioBackground.pause();
    // };
  },
  computed: mapState(useGameStore, ['isStart', 'task', 'loading', 'game', 'key', 'coins', 'isGetCoins', 'numCoins', 'isGetCards', 'account', 'correctAnswer']),
  watch: {
    task: {
      handler(task) {
