...
function add_form() {
	...
	loadParentCategory();
	$('.modal-title').text('Thêm danh mục '+language['type_'+category_type]);
		...
		success: function(data) {
			$.each(data.data, function( key, value ) {
				$.each(value, function( k, v) {
					if($.inArray(k,['title','meta_title','description','meta_description','slug','content','thumb_home','banner_adv','meta_keyword','address']) !== -1){
						$('[name="'+k+'['+value.language_code+']"]').val(v);
						if(tinymce.get(k+'_' + value.language_code)) tinymce.get(k+'_' + value.language_code).setContent(v);
						if(k=='banner_adv' || k=='thumb_home') loadImageThumb(v,k+'['+value.language_code+']');
					}else{
						$('[name="'+k+'"]').val(v);
					}
				});
			...
			/*Load category parent*/
			$('#modal_form').modal('show');
			$('.modal-title').text('Sửa danh mục '+language['type_'+category_type]);
		},
